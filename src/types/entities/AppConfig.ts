...
export interface AppConfig {
  ...
  /**
   * Max combined element count (nodeCount + edgeCount) allowed for opening in
   * Cytoscape Web. Above this, the "Open in Cytoscape Web" option is disabled.
   * (optional, defaults to 26000)
   */
  maxNetworkElementsThreshold?: number
  /**
   * Max edgeCount allowed for opening in Cytoscape Web. Above this, the
   ...
   * is used as-is. Either way it must resolve to the app's own origin — a
   * cross-origin endpoint is a configuration error, reported to the console and
   * not sent, because its response cannot be read. Intended to point at a resource-less endpoint the web server
